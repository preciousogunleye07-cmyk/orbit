...
export const Navbar: React.FC<NavbarProps> = ({ setActiveModal, currentPage, setCurrentPage }) => {
  ...
  return (
    ...
    >
      ...
      {/* Mobile Drawer */}
      {mobileMenuOpen && (
        <div className="md:hidden bg-[#1c1b1b] border-b border-[#353434] px-6 pt-4 pb-6 mt-3 space-y-2 shadow-2xl animate-fade-in">
          {[
            { id: 'home', label: 'Home' },
            ...
            <button
              ...
              className={`block w-full text-left px-4 py-2.5 rounded-lg font-medium text-xs ${
          ...
          <div className="pt-3 border-t border-[#353434] flex flex-col gap-2">
            <button
              ...
              className="w-full py-2.5 rounded-full bg-[#ffffff] text-[#141313] font-bold text-center text-xs shadow-sm"
            >
              Enroll in a Program
            </button>
            <button
              onClick={openWhatsApp}
              className="w-full py-2.5 rounded-full bg-[#201f1f] border border-[#353434] text-[#e5e2e1] font-medium text-center text-xs flex items-center justify-center gap-2"
            >
