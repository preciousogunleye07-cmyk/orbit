...
export const AdminTimetableManager: React.FC<AdminTimetableManagerProps> = ({
  ...
  const [isTutorsListPickerOpen, setIsTutorsListPickerOpen] = useState<boolean>(false);
  const [allTutors, setAllTutors] = useState<TutorProfile[]>(() => TutorService.getAllTutors());

  useEffect(() => {
    const unsubTutors = TutorService.subscribeTutors((tutors) => {
      setAllTutors(tutors);
    });
    return () => unsubTutors();
  }, []);
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2.5 flex-wrap w-full sm:w-auto">
            ...
            <button
              onClick={() => {
                playSound('droplet');
                setIsTutorsListPickerOpen(true);
              }}
              className="px-3.5 py-2 rounded-xl bg-[#1d172e] hover:bg-[#281f3e] border border-[#a855f7]/40 hover:border-[#a855f7] text-xs text-[#c084fc] hover:text-white flex items-center gap-1.5 transition-all cursor-pointer shadow-sm"
              title="Edit tutor names and details across the entire system"
              id="btn-admin-manage-tutors"
            >
              <User className="w-3.5 h-3.5 text-[#a855f7]" />
              <span>Edit Tutors</span>
            </button>
      ...
      <AnimatePresence>
        ...
        {/* Tutor Management Quick Picker Modal */}
        {isTutorsListPickerOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm overflow-y-auto">
            <motion.div
              initial={{ opacity: 0, scale: 0.95 }}
              animate={{ opacity: 1, scale: 1 }}
              exit={{ opacity: 0, scale: 0.95 }}
              className="bg-[#161224] border border-[#3e345a] rounded-3xl max-w-lg w-full p-6 shadow-2xl space-y-4 my-8"
            >
              <div className="flex items-start justify-between gap-3 border-b border-[#2e2642] pb-3">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <h3 className="text-base font-bold text-white">Orbit Space Tutors</h3>
                    <p className="text-xs text-[#94a3b8]">Select any tutor below to edit their name and profile.</p>
                  </div>
                </div>
                <button
                  type="button"
                  onClick={() => setIsTutorsListPickerOpen(false)}
              ...
              <div className="space-y-2 max-h-96 overflow-y-auto pr-1">
                {allTutors.map((tutor) => (
                  <div
                    key={tutor.id}
                    className="p-3 rounded-2xl bg-[#1d172e] border border-[#31284a] hover:border-[#a855f7]/50 flex items-center justify-between gap-3 transition-colors group"
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="w-8 h-8 rounded-full bg-purple-500/20 border border-purple-500/30 flex items-center justify-center text-purple-200 font-bold text-xs shrink-0">
                        {tutor.avatar || tutor.shortName.charAt(0)}
                      </div>
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          <h4 className="text-xs font-semibold text-white truncate">{tutor.name}</h4>
                          <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-purple-900/40 text-purple-300 border border-purple-700/40">
                            {tutor.shortName}
                          </span>
                        </div>
                        <p className="text-[11px] text-[#a19cb5] truncate mt-0.5">{tutor.role || tutor.specialization}</p>
                      </div>
                    </div>
                    <button
                      type="button"
                      onClick={() => {
                        ...
                        setIsTutorsListPickerOpen(false);
                        setIsTutorModalOpen(true);
                      }}
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-purple-600/20 hover:bg-purple-600/40 border border-purple-500/30 text-xs font-medium text-purple-300 hover:text-white transition shrink-0 cursor-pointer"
                    >
                      <Edit3 className="w-3.5 h-3.5" />
                      <span>Edit</span>
                    </button>
              ...
              <div className="pt-2 border-t border-[#2e2642] flex justify-end">
                <button
                  type="button"
                  onClick={() => setIsTutorsListPickerOpen(false)}
                  className="px-4 py-2 rounded-xl bg-[#1e192c] hover:bg-[#28213a] text-xs text-[#c4c7c8] hover:text-white transition cursor-pointer"
                >
                  Close
                </button>
              </div>
        ...
        {/* Edit Tutor Modal for Global Propagation */}
        {isTutorModalOpen && tutorToEdit && (
          <EditTutorModal
            tutor={tutorToEdit}
            isOpen={isTutorModalOpen}
            onClose={() => setIsTutorModalOpen(false)}
            onTutorUpdated={(updated) => {
              showToast(`Tutor "${updated.name}" updated! Changes applied to timetable and attendance.`);
              setAllTutors(TutorService.getAllTutors());
              // Refresh slots from cache to reflect cascading name changes
              setSlots(getLocalTimetableSlots());
            }}
          />
        )}
